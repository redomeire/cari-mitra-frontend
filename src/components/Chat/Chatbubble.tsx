...
const Chatbubble = ({ item, partner, socket, setMessages, isPartner }: {
    item: {
        id: number,
    ...
    return (
        ...
        >
            {
                isPartner ?
                    <ChatBubble end={item.sent_by_partner ? true : false}>
                        ...
                        <ChatBubble.Message color="primary" className={item.sent_by_partner ? 'pl-10' : ''}>
                            {
                                item.sent_by_partner ?
                                    <Dropdown className={`absolute left-2 top-2 z-50`}>
